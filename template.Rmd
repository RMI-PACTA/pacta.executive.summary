...
data_diagram <- prep_diagram(audit_data = NULL, emissions_data = NULL)
...
# the specific PACTA outputs that we need here are my guesses 
data_scatter_power <- prep_scatter(results_portfolio = NULL, peers_results_individual = NULL, 
                                     indices_results_portfolio = NULL, sector = "power", asset_class = "bonds")
plot_scatter(data_scatter_power) + 
  patchwork::plot_annotation(
    title = "Bonds",
    theme = theme(plot.title = element_text(face = "bold", hjust = 0.5, size = 22))
...
```{r scores_equity}
# the specific PACTA outputs that we need here are my guesses 
data_scores <- prep_scores(results_portfolio = NULL, peers_results_aggregated = NULL,
                           asset_class = "equity")
...
```{r}
#TODO: add code here for plotting exposure
```
