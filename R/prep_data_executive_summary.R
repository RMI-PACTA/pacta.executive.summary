...
#' @param es_main_dir Some description
#'
#' @return data.frame
#' @export
#'
prep_data_executive_summary <- function(investor_name,
  ...
  emissions_indices_eq <- readRDS(file.path(es_main_dir, "Indices_equity_emissions.rds")) %>%
    dplyr::mutate(entity = "benchmark") %>%
    dplyr::filter(grepl("MSCI World", .data$portfolio_name))

  emissions_indices_cb <- readRDS(file.path(es_main_dir, "Indices_bonds_emissions.rds")) %>%
    dplyr::mutate(entity = "benchmark") %>%
    dplyr::filter(grepl("Global Corp Bond", .data$portfolio_name))
  ...
  audit_indices_eq <- readRDS(file.path(es_main_dir, "Indices_equity_audit.rds")) %>%
    dplyr::mutate(entity = "benchmark") %>%
    dplyr::filter(grepl("MSCI World", .data$portfolio_name))

  audit_indices_cb <- readRDS(file.path(es_main_dir, "Indices_bonds_audit.rds")) %>%
    dplyr::mutate(entity = "benchmark") %>%
    dplyr::filter(grepl("Global Corp Bond", .data$portfolio_name))
