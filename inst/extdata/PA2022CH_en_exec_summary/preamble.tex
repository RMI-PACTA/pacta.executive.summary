...
\mdfdefinestyle{scheaderstyle}{
  frametitlebackgroundcolor = scBlueGray,
  fontcolor=white
}
...
\NewDocumentCommand \scheaderTitleString{ m o }{
  % \Large#1\vrule{24pt}{2pt}%
  \Large#1%
  \IfValueT{#2}{\hfill {#2}}
}
...
\NewDocumentEnvironment{scheader}{ O{\textwidth} m o }
{% begin code
  \begin{minipage}[t]{#1}%
  \begin{mdframed}[style=scheaderstyle, frametitle={\scheaderTitleString{#2}[#3]}]%
}
{ % enddef
  \end{mdframed}%
  \end{minipage}%
