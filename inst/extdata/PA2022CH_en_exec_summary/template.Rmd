...
```
Portfolio emissions covered by assessment: **X %**
...
fin_data_net_zero_targets <- readr::read_csv(
  file.path(survey_dir, language, "fin_data_net_zero_targets.csv"),
...
peer_group_share_net_zero <- readr::read_csv(
  file.path(survey_dir, language, "peer_group_share_net_zero.csv"),
  col_types = readr::cols_only(
    investor_name = "c",
    portfolio_name = "c",
    share_net_zero = "n"
...
### Exposure to companies that have at least some fossil fuel and low-carbon activities
...
By substituting fossil with renewable heating systems and energy efficient refurbishments of buildings, a direct and impactful climate contribution can be achieved. Real estate and mortgage are therefore important asset classes. **The graph show…**
...
* PACTA measures what firms plan to do over the next 5 years instead of what they promise
